...
def build_queries(name=None, email=None, phone=None, town=None, age=None, associate=None):
    ...
    if name:
        ...
        add("X (Twitter) — profile check", f"https://www.google.com/search?q={q}+site:x.com")
        ...
        if town:
            ...
            age_suffix = f"+age+{quote_plus(str(age))}" if age else ""
    ...
    if email:
        ...
        add("Reverse email reputation lookup", f"https://www.google.com/search?q=email+reputation+lookup+{qe}")
        add("Google — exact email (breach/exposure check)", f"https://www.google.com/search?q={qe_exact}")

    if phone:
        qp = quote_plus(phone)
        qp_exact = quote_plus(f'"{phone}"')
        add("Reverse phone reputation lookup", f"https://www.google.com/search?q=phone+reputation+lookup+{qp}")
...
def main():
    ...
    ap.add_argument("--age", help="Subject age (narrows obituary search)")
